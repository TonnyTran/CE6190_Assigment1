...
def prox_linf_indicator(δ: Tensor, λ: Tensor, lower: Tensor, upper: Tensor, ε: float = 1e-6,
    ...
    δ_proj = torch.minimum(δ_, lower.flatten(1))
    δ_proj = torch.minimum(δ_proj, upper.flatten(1))
    ...
    for _ in range(steps):
        ...
        prox = torch.minimum(δ_proj, -left_third)
        prox = torch.maximum(prox, left_third)
        f_left = torch.sum(prox.sub_(δ_).square_(), dim=1, keepdim=True, out=f_left)
        f_left.div_(2).addcmul_(left_third, λ_)

        prox = torch.minimum(δ_proj, -right_third)
        prox = torch.maximum(prox, right)
    ...
    prox = torch.minimum(δ_proj, -left)
    prox = torch.maximum(prox, left).view_as(δ)
    return prox
...
def init_lr_finder(grad: Tensor, norm: float, target_distance: float, lower: Tensor, upper: Tensor) -> Tensor:
    ...
    found_upper = torch.minimum(grad.neg(), lower)
    found_upper = torch.maximum(found_upper, upper).flatten(1).norm(p=norm, dim=1) > target_distance
    while (~found_upper).any():
        low = torch.where(found_upper, low, lr)
        lr = torch.where(found_upper, lr, lr * 2)
        found_upper = torch.minimum(grad.mul(-batch_view(lr)), lower)
        found_upper = torch.maximum(found_upper, upper).flatten(1).norm(p=norm, dim=1) > target_distance

    for i in range(20):
        new_lr = (low + lr) / 2
        larger = torch.minimum(grad.mul(-batch_view(new_lr)), lower)
        larger = torch.maximum(larger, upper).flatten(1).norm(p=norm, dim=1) > target_distance
